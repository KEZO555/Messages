...
private fun DeviceKeyboardTextInputEditor(
    ...
    // The IME must resize the window rather than pan it, or the bottom bar and
    // the composer's clear-draft X end up underneath the keys. LightOS tool
    // activities are generated by the SDK plugin, so the mode can't be set in
    // the manifest — set it while the editor is up and put it back on the way
    // out, leaving every other screen as the SDK configured it.
    ...
    DisposableEffect(activity) {
        ...
        onDispose {
            if (window != null && previous != null) {
                runCatching { window.setSoftInputMode(previous) }
            }
        }
    }
...
/** The hosting activity, for the window's soft-input mode. */
